#ifndef UMINEKO_SPARSE_TRSV_DIRECT_HPP
#define UMINEKO_SPARSE_TRSV_DIRECT_HPP
...
template <class, spmat::Form> struct Direct;

namespace l {
template <class C> using Direct = Direct<C, spmat::Form::lower>;
}
namespace ld {
template <class C> using Direct = Direct<C, spmat::Form::lower_d>;
}
namespace du {
template <class C> using Direct = Direct<C, spmat::Form::d_upper>;
}

template <typename T, class L, spmat::Form form>
struct Direct<CSR<T, L>, form> : private CSR<T, L> {
  static_assert(std::is_same_v<L, host>, "Direct<CSR> does not work on device");
  using CSR<T, L>::val, CSR<T, L>::idx, CSR<T, L>::rptr;

  template <typename _>
  explicit Direct(const CSR<_, host> &in)
      : CSR<T, L>([&] {
  ...
  void solve(const vector<in_t, L> &in, vector<out_t, L> out) const {
    if constexpr (form == spmat::Form::lower) {
      exec<L>::seq_for(out.size(0), [=, *this](idx_t i) mutable {
        auto t = in[i];
        auto j = rptr[i];
        for (; j < rptr[i + 1] - 1; ++j)
          t -= val[j] * out[idx[j]];
...
struct Direct<BCSR<bnl, bnw, T, L>, form> : private BCSR<bnl, bnw, T, L> {
  static_assert(std::is_same_v<L, host>, "Direct<BCSR> does not work on device");
...
template <class C, spmat::Form S> struct is_trsv<trsv::Direct<C, S>> {
